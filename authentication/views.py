...
class EmailValidationView(View):
    def post(self, request):
        ...
        #if not validate_email(email):
            return JsonResponse({'email_error': 'Некорректный email'}, status=400)
        if User.objects.filter(email=email).exists():
            return JsonResponse({'email_error': 'Извините, email уже занят'}, status=409)
    ...
    def post(self, request):
        ...
        if username and password:
            ...
            if user:
                ...
                messages.error(request, 'Аккаунт не активирован. Пожалуйста, укажите Ваш email.')   
                return render(request, 'authentication/login.html') 
        
            messages.error(request, 'Неверно указаны данные учетной записи')   
            return render(request, 'authentication/login.html')      
